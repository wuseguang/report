...
(function($){
	...
	$.fn.extend({
		...
		ajaxUrl: function(op){
			...
			var dataType=op.data.dataType;//update
			delete op.data.dataType;
			$.ajax({
				type: op.type || 'GET',
				url: op.url,
				data: op.data,
				...
				success: function(response){
					...
					response=dataType=='json'||op.type=='POST'?JSON.stringify(response):response;
					var pdata=$.extend(getUrlParams(op.url),op.data);
					response=response.injectJSON(pdata,'#');
					//alert(response)
					...
					if(json.notlogin)DWZ.loadLogin();
					else if (json.statusCode==DWZ.statusCode.timeout){
