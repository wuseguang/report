...
function initUI(_box){
	...
	$("a[target=navTab],button[target=navTab]", $p).each(function(){
		$(this).click(function(event){
			...
			var data=$this.attr('data')||'{}';//update
			data=JSON.parse(data);//updata
			...
			navTab.openTab(tabid, url,{title:title, fresh:fresh, dataType:dataType,external:external,data:data});

			event.preventDefault();
		});
	});
	//立即加载项
	$('[loadNow]',$p).each(function(){
		$(this).loadUrl($(this).attr('href'),{},null);
	})
	//dialogs
	$("a[target=dialog]", $p).each(function(){
		$(this).click(function(event){
			...
			options.data=JSON.parse($this.attr('data')||'{}');//update
			var url = unescape($this.attr("href")).replaceTmById($(event.target).parents(".unitBox:first"));
			DWZ.debug(url);
			if (!url.isFinishedTm()) {
				alertMsg.error($this.attr("warn") || DWZ.msg("alertSelectMsg"));
	...
	$("a[target=ajax]", $p).each(function(){
		$(this).click(function(event){
			...
			var postData=JSON.parse($this.attr('data')||'{}')//update
			if (rel) {
				var $rel = $("#"+rel);
				$rel.loadUrl($this.attr("href"), postData||{}, function(){//update
	...
	// 这里放其他第三方jQuery插件...
	$p.children().each(function(){$(this).initDataWidget()});
	//$p.children('.dataWidget').each(function(){$(this).dataWidget()});
}
