...
function divSearch(form, rel){
	var $form = $(form);
	if (form[DWZ.pageInfo.pageNum]) form[DWZ.pageInfo.pageNum].value = 1;
	//console.log(form[DWZ.pageInfo.pageNum].value)
	if (rel) {
		var $box = $("#" + rel);
...
function _getPagerForm($parent, args) {
	...
	if(!form)form=$("#pagerForm", $parent.parent()).get(0)
	if (form) {
...
function dwzPageBreak(options){
	...
	if (op.rel) {
		...
		if (form) {
			//update if
			if($box.hasClass('dataWidget')){
				$box.attr('widgetData',JSON.stringify($(form).serializeArray()));
				$box.dataWidgetUpdate();
				return;
			}
			$box.ajaxUrl({
				type:"POST", url:$(form).attr("action"), data: $(form).serializeArray(), callback:function(){
...
function departSearchSubmit(form){
	$(form).pagerForm({parentBox:$(form).parent()});
	var data=$(_getPagerForm($(form).parent(),{})).serializeArray()
	$('#depart_search_table').attr('widgetData',JSON.stringify(data)).dataWidgetUpdate();
	return false;
}
function userSearchSubmit(form){
	$(form).pagerForm({parentBox:$(form).parent()});
	var data=$(_getPagerForm($(form).parent(),{})).serializeArray()
	$('#user_search_table').attr('widgetData',JSON.stringify(data)).dataWidgetUpdate();
	return false;
}
