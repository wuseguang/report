	...
	$.fn.extend({
		pagination: function(opts){
			...
			function _bindEvent($target, pageNum, targetType, rel){
				//alert($target.text()+targetType+rel)
		...
		pagerForm: function(options){
			...
			return this.each(function(){
				...
				//update查找最近的pagerForm
				var currentParent=op.parentBox;
				while($pagerForm.length==0&&currentParent.length>0){
					currentParent=currentParent.parent();
					$pagerForm=$(op.pagerForm$, currentParent);
				}//update
				//console.log($pagerForm.html())
				if($pagerForm.length==0)return;//update
				...
				$searchForm.find(":input").each(function(){
					...
					if (name && (!$input.is(":checkbox,:radio") || $input.is(":checked"))){
						var $pagerFormInput=$pagerForm.find(":input[name='"+name+"']");
						if ($pagerFormInput.length == 0) {
							var inputFrag = frag.replaceAll("#name#", name).replaceAll("#value#", $input.val());
							$pagerForm.append(inputFrag);
						}else{//update
							$pagerFormInput.val($input.val());
						}
					}
				});
			});
