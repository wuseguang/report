 ...
 */
(function($){
	var _lookup = {currentGroup:"", suffix:"", $target:null, pk:"id",pkMap:{}};//update pkmap
	var _util = {
		...
		_realKey:function(key){
			return _lookup.pkMap[key]||key;
		}
	};
	...
	$.extend({
		bringBackSuggest: function(args){
			...
			$box.find(":input").each(function(){
				...
				var noMatchTitle=$input.attr('noMatchTitle')||'没有匹配的选项';
				for (var key in args) {
					var oldKey=key;
					key=_util._realKey(key);//update
					var name = (_lookup.pk == key) ? _util.lookupPk(key) : _util.lookupField(key);
					
					if (name == inputName&&args[oldKey]!=noMatchTitle) {
						//console.log(name+':'+inputName)
						//console.log(args[oldkey])
						$input.val(args[oldKey]);
						break;
					}
				}
			});
			...
			return this.each(function(){
				...
				function _show(event){
					...
					_lookup = $.extend(_lookup, {
						...
						pkMap:JSON.parse($input.attr('pkMap')||'{}')
					});
					...
					var postData = {};
					postData[$input.attr("postField")||$input.attr('name')] = $input.val();// update "inputValue"
					var resultField=$input.attr("resultField");
					var suggestFormat=$input.attr('suggestFormat');
					$.ajax({
						global:false,
						...
						success: function(response){
							if (!response) return;
							if(resultField)response=response[resultField];
							if(response.length==0)response=[$input.attr('noMatchTitle')||'没有匹配选项'];
							var html = '';
							...
							$.each(response, function(i){
								...
								//update
								if($.type(this)=='string'){
									liAttr=$input.attr('name')+":'"+this+"'";
									liLabel=this;
									html += '<li lookupAttrs="'+liAttr+'">' + liLabel + '</li>';
									return;	
								}
								if(suggestFormat)liLabel=suggestFormat.injectJSON(this,'%');//add if update
								else
							...
							if ($lis.size() == 1 && event.keyCode != DWZ.keyCode.BACKSPACE&!$input.attr('canEdit')) {
								_select($lis.eq(0));
							} else if ($lis.size() == 0){
								var jsonStr = "";
								for (var i=0; i<suggestFields.length; i++){
